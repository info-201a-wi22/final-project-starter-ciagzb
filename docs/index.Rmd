...
## Summary
By using plots and tables we have made, we are able to learn different countries' different wage situations. For example, among Norway, USA, and Mexico in 2005, 2010, 2015, 2020, we have found that the US has the widest gap while Norway has the least gap percentage, and Mexico has the largest decrease in gender wage gap. The result of the gender wage gap shows different trend from HDI ranking, which means that well-developed countries may still has room for improvement in gender equality. Such conclusion can also be found in other plots, and more results can be noticed from other plots.
...
## Aggregate Table
```{r, echo=FALSE}
p02_code(For Cleaning up data and Calculations)
```
